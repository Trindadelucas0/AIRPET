...
const Usuario = {
  ...
  async atualizarPerfil(id, dados) {
    ...
    const colunasPermitidas = ['nome', 'telefone', 'cor_perfil', 'bio', 'apelido', 'endereco', 'bairro', 'cidade', 'estado', 'cep', 'data_nascimento', 'contato_extra', 'foto_perfil', 'foto_capa', 'receber_alertas_pet_perdido'];
    const setPartes = [];
    const params = [id];
    let idx = 2;
    colunasPermitidas.forEach((col) => {
      ...
      else if (col === 'data_nascimento' || col === 'contato_extra' || col === 'bio' || col === 'apelido' || col === 'endereco' || col === 'bairro' || col === 'cidade' || col === 'estado' || col === 'cep') params.push(dados[col] || null);
